...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
        ...
        def normalized_tanh(x):
            ...
            t = T.tanh(x)
            return t/T.sqrt(T.sum(t**2))

        self.f = normalized_tanh

        def cost_update(wrong_ans, prev, corr_ans, h_n):
            base = 1 - T.dot(corr_ans, h_n)
            return prev + T.maximum(0., base + T.dot(wrong_ans, h_n))
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            ...
            '''
            # raise NotImplementedError
            # The value of hidden_states[n]
            h_n = self.f(T.dot(self.Wv, x[n]) + self.b + hidden_sums[n])

            # The updated value of hidden_sums[p[n]]
            h_sums = T.set_subtensor(hidden_sums[p[n]], hidden_sums[p[n]] + T.dot(r[n], h_n))

            # The updated cost
            new_cost, updates = theano.scan(fn=cost_update,
                                            sequences=wrong_ans,
                                            outputs_info=T.as_tensor_variable(np.asarray(0, theano.config.floatX)),
                                            non_sequences=[corr_ans, h_n])

            costs = new_cost[-1] + cost
            return T.set_subtensor(hidden_states[n], h_n), h_sums, costs
